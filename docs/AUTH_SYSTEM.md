...
| 제공자 | 채택 | 이유 |
|--------|------|------|
| **Google** | ✅ | 글로벌 1위, 한국 점유율 높음, Supabase 완벽 지원 |
| **Apple** | ✅ | 아이폰 사용자 필수, Supabase 완벽 지원 |
| Kakao | ❌ | OIDC 연동 불안정, Supabase 미지원 |
| Naver | ❌ | Supabase 미지원, OIDC 커스텀 연동 복잡 |
| 이메일/비밀번호 | ✅ | Supabase 기본 지원, 이메일 인증 포함 |

### 1.2 결론
**Google + Apple + 이메일** 3종으로 충분. 한국 사용자의 90% 이상 커버.
